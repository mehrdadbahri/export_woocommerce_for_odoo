...
"on a.ID = m.object_id "\
"left join "\
"(SELECT post_id, meta_value FROM ${db_name}.${wp_db_prefix}_postmeta where meta_key = \"_product_attributes\") as n "\
"on a.ID = n.post_id "\
"where a.post_type = \"product\" and a.post_status != \"trash\" and h.meta_value = \"yes\";"\
| sed 's/\t/ة/g' > variant_products_temp.csv

out_file=variant_products.csv
in_file=variant_products_temp.csv
echo \"id\",\"name\",\"website_description\",\"website_published\",\"default_code\",\"lst_price\",\"product_brand_id/id\",\
	\"sale_ok\",\"purchase_ok\",\"type\",\"website_meta_title\",\"website_meta_description\",\
	\"website_public_categ_ids\",\"attributes\",\
	\"image\",\"images/image\" > $out_file;
	i=1
	line=1
	NUMOFLINES=$(wc -l < "$in_file")

	while IFS="ة" read f1 f2 f3 f4 f5 f6 f7 f8 f9 f10 f11 f12 f13 f14 f15 f16 f17 f18 f19 f20 f21 f22 f23 f24 f25 f26 f27 f28 f29 f30 f31 f32 f33 f34 f35; do
		if (($i))
		then
			i=0
			continue
